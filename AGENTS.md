...
- `internal/cmd/`: Command implementations using kong structs. Each command group has its own file(s): `mail_*.go`, `calendar*.go`, `todo.go`, `todo_checklist.go`, `todo_attachments.go`, `todo_links.go`, `whoami.go`, etc. `token.go` implements token mode (credential injection) and `root.go` wires it into `RunContext`.
...
- Existing coverage: `internal/outfmt` (formatting + untrusted-wrapping), `internal/config`, `internal/msauth/scopes`, `internal/graphapi` (validate + capability guards), and `internal/cmd` (paging filters + mailbox-target validation, MCP server/registry, schema generation, argv building, output capture, token mode in `token_test.go`). Coverage is partial — many command and Graph-wrapper paths remain untested.
...
- The `package` job runs `scripts/test-npm-package.sh` — builds a host binary, stamps + packs the npm wrapper packages, installs the tarballs, and runs `olk version` through the JS launcher in both unscoped and scoped layouts. This makes npm packaging breakage a PR failure instead of a tag-time failure. It also runs `scripts/test-bootstrap-npm.sh`, which covers the publish-argument shape: absolute tarball paths (npm parses a bare relative path as a `<owner>/<repo>` git shorthand), an explicit dist-tag on every publish (npm ≥ 11 rejects an untagged prerelease, and every fork version is a prerelease), the publish order, and the tarball-set validation. Both scripts restore `npm/*/package.json` on exit; the job then asserts a clean working tree.
...
- **Token mode (credential injection)** — `internal/cmd/token.go`. When `--access-token` / `OLK_ACCESS_TOKEN` is set, olk becomes a pure consumer of an externally minted **delegated Graph access token**: it never touches the keyring, the account files, or the default-account config; it never refreshes; it never persists the token. The token's lifetime is the process's lifetime. This exists so a disposable environment (CI, container, agent sandbox) holds no durable refresh token — a container-local keyring would both leak a long-lived credential and race Microsoft's refresh-token rotation across concurrent containers.
  ...
  - `auth login|logout|clean|list` refuse to run in this mode (they manage *stored* accounts); `auth status` reports the injected token. `--account` is rejected; `--mailbox` still works. Every capability guard composes normally.
  - The token must never reach output, errors, verbose HTTP logs (the `Authorization` header is redacted), or the argv the MCP server builds. `token_test.go` asserts this — keep it true when touching logging or MCP argv construction.
  - This is the mechanism the fork's npm distribution is built for: `npx @planmonster/olkcli` in a sandbox, credentials injected by the control plane. See the README section "Access-Token Injection" and `docs/npm-publishing.md`.
