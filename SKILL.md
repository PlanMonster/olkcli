...
| — | `OLK_ACCESS_TOKEN` | Environment-only injected Graph access token instead of a stored login: no CLI flag, keyring, refresh, or persistence. `--account` is rejected in this mode; `--mailbox` still works |
...
| `--account-email EMAIL` | `OLK_ACCOUNT_EMAIL` | Backward-compatible, non-authoritative identity hint shown by `olk auth status`; display only, never authorization |
